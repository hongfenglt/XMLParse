...
-(NSString*)soapMessageResultXml:(NSString*)methodName xmlData:(NSString**)content{
    if (self.document) {
        //获取根节点
...
-(XmlNode*)selectSingleNode:(NSString*)xpath nameSpaces:(NSDictionary*)spaces{
    if (self.document) {
        ...
        if (childs&&[childs count]>0) {
            ...
            XmlNode *entity=[[XmlNode alloc] init];
...
-(XmlNode*)xmlStringToXmlNode{
    if (self.document) {
        GDataXMLElement* rootNode = [self.document rootElement];
        XmlNode *parent=[[XmlNode alloc] init];
        ...
        for (GDataXMLNode *node in rootChilds) {
            XmlNode *item=[[XmlNode alloc] init];
